...
def sinkhorn_loop(log_alpha, log_beta, cost_xy, cost_yx, cost_xx, cost_yy, epsilon, particles_diameter, scaling,
    ...
    # final_a_y = softmin(epsilon, cost_yx, log_alpha + tf.stop_gradient(converged_b_x) / epsilon)
    # final_b_x = softmin(epsilon, cost_xy, log_beta + tf.stop_gradient(converged_a_y) / epsilon)
    # final_a_x = softmin(epsilon, cost_xx, log_alpha + tf.stop_gradient(converged_a_x) / epsilon)
    # final_b_y = softmin(epsilon, cost_yy, log_beta + tf.stop_gradient(converged_b_y) / epsilon)

    return converged_a_y, converged_b_x, converged_a_x, converged_b_y, total_iter
    # return final_a_y, final_b_x, final_a_x, final_b_y, total_iter + 1
